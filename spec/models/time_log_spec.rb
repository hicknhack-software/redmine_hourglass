  ...
  describe 'booking' do
    ...
    context 'with no extra arguments' do
      it 'tries creating a time booking with the correct arguments' do
        expect(Chronos::TimeBooking).to receive(:create).with({start: time_log.start, stop: time_log.stop, time_log_id: time_log.id, time_entry_arguments: {comments: time_log.comments, user: time_log.user, spent_on: time_log.start.to_date, hours: hours}}.with_indifferent_access)
        book!
      end
    end
